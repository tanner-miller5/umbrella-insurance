...
import { callCreateTeamRecordRestEndpoints, callReadTeamRecordRestEndpointsByTeamIdAndSeasonId, callUpdateTeamRecordRestEndpoints } from "../../../../../../endpoints/rest/records/teamRecords/v1/TeamRecordRestEndpoints";
...
import { callCreateRecordRestEndpoints } from "../../../../../../endpoints/rest/records/v1/RecordRestEndpoints";
...
var env: string = "TEST";
describe.skip('team record endpoint tests', () => {
    var teamRecordId: number | undefined; 
    ...
    var teamRecord: TeamRecord = new TeamRecord();

    var updatedTeamRecord: TeamRecord = new TeamRecord();

    var domain: string = "http://localhost:8080";

    test('call create team record', async () => {
        ...
        updatedTeamRecord.season = season;
        updatedTeamRecord.team = team;
        updatedTeamRecord.record = record;
        
        var teamRecordResponse: TeamRecord = await callCreateTeamRecordRestEndpoints(
            teamRecord, env, domain);
        teamRecordId = teamRecordResponse.id;
        updatedTeamRecord.id = teamRecordResponse.id;
        expect(teamRecord.team).toBe(teamRecordResponse.team);
        expect(teamRecord.record).toBe(teamRecordResponse.record);
        expect(teamRecord.season).toBe(teamRecordResponse.season);
    });

    test('call read team record', async () => {
        var teamRecords: TeamRecord[] | undefined = await callReadTeamRecordRestEndpointsByTeamIdAndSeasonId(
            team?.id || 1, season.id || 1, env, domain) || [];
        expect(teamRecords[0].team).toBe(teamRecord);
        expect(teamRecords[0].record).toBe(teamRecord.record);
        expect(teamRecords[0].season).toBe(teamRecord.season);
    });

    test('call update team record', async () => {
        var teamRecordResponse: TeamRecord[] = await callUpdateTeamRecordRestEndpoints(
            updatedTeamRecord, env, domain);
        expect(updatedTeamRecord.team).toBe(teamRecordResponse[0].team);
        expect(updatedTeamRecord.record).toBe(teamRecordResponse[0].record);
        expect(updatedTeamRecord.season).toBe(teamRecordResponse[0].season);
    });

    test('call delete team record', async () => {
        var deleteTeamResponse: boolean = await callDeleteTeamRestEndpointsByTeamId(
            teamRecordId || 1, env, domain);
